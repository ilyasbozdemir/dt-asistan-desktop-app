    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
      <div className="border-b border-slate-100 dark:border-slate-800 pb-2">
        ...
        <p className="text-xs text-slate-500">
          Alım süreçlerindeki varsayılan işlem aşamaları.
        </p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {asamalar.map((asama) => (
            <div key={asama.id} className="p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl">
              <div className="flex items-center gap-3 mb-2">
                <span className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-white bg-${asama.rozet_rengi}-500 shrink-0`}>
                  {asama.asama_sira}
                </span>
